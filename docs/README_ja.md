...
`openat` により認証情報ファイルへのアクセス（`.ssh/`、`.aws/`、`/etc/shadow`）を検知し、`rename` により信頼されたバイナリの差し替えを検知します。DNS トンネリング検知は `sendto` ペイロードからクエリドメインを抽出し、高エントロピーなサブドメインによるデータ流出を検出します。`sendfile` はフォレンジック目的で trace に含まれますが、構造化イベントとしてはパースされません。
