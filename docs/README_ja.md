...
| サンプル | ソース | 検知内容 |
|----------|--------|----------|
| axios-attack-demo | testdata | C2 通信、認証情報窃取（SSH, AWS, Git 等 6 種）、ペイロード実行 |
| a1rn | [Datadog](https://github.com/DataDog/malicious-software-packages-dataset) | `curl` によるファイル外部送信 |
| advpruebitaa | Datadog | シェルコマンドによるファイル作成・ディレクトリ列挙 |
| 0wneg 他 5 件 | Datadog | インストール時の `python3 -c` インラインコード実行 |
