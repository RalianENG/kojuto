...
3. **インストール監視** — `connect`, `sendto`, `sendmsg`, `sendmmsg`, `bind`, `listen`, `accept`, `execve`, `openat`, `rename`, `mmap`, `mprotect`, `unlink`, `sendfile`, `ptrace` syscall を記録
...
- **Python の `eval`/`exec`、Node.js の `Function()`** — インタープリタ内部実行のため `execve` syscall が発生しない
