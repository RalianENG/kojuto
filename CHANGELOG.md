...
### Added
- **`CategoryUnknownBinary` (severity LOW)** — execve events that do not match a positively-defined attack signature (suspicious path, inline `-c`/`-e` flag, sh -c failing `isShellCmdBenign`) are recorded under this new category for forensic chain visibility without flipping the verdict. The detection rationale block in `classifyExecve` documents why a positive allowlist of legitimate-install binaries is intractable and why the harm-firing syscall layer (network/credential/persistence/RWX/audit hook) is the actual detection point
- **V8 JIT mprotect/mmap filter** — analyzer pre-pass builds a PID→comm map from execve events, then skips simultaneous-RWX `mprotect`/`mmap` events whose PID resolves to a known JIT interpreter (`node`/`nodejs`/`deno`/`bun`). The filter requires a real prior execve for the PID, so an attacker cannot bypass by spoofing PIDs in the strace stream. Eliminates the per-package `memory_execution` false positive that every Node-driven scan previously produced. The long-standing TODO comment in `strace_parse.go` documenting this issue is now retired
...
### Changed
- **Probe install launch uses a staged script file, not `sh -c <inline>`** — `Sandbox.InstallCommand` / `InstallAllCommand` now write the install script to `/var/cache/kojuto/install.sh` (a dedicated tmpfs) via `dockerWriteFile` and return `["sh", "/var/cache/kojuto/install.sh"]`. The outer probe shell is filtered as benign by `isBenignExec` (sh from `/bin/` matches `benignPaths`) instead of tripping the `sh -c` HIGH-severity branch of `classifyExecve`. Attackers cannot mimic this shape because npm/yarn/pnpm always spawn lifecycle hooks as `sh -c <package script>`; the file-path form is reserved for kojuto's own launch path. Signature change: both methods now take `context.Context` and return `(cmd, error)`
- **Unrecognized execve demoted to LOW severity** — `classifyExecve`'s default branch (binary not matching `/dev/shm`-class paths, inline interpreter flags, or `sh -c` benign-check failures) now assigns `CategoryUnknownBinary` instead of `CategoryCodeExecution`. The event still appears in the report for chain visibility, but verdict is decided by the syscall-level rules that observe the binary's actual behavior. This eliminates the noise from compiler-toolchain / probe-scaffolding execve events that all reached the same HIGH bucket without a positive attack signature. A detailed design rationale lives above `classifyExecve` documenting the dynamic/static defense split and what is intentionally moved off the verdict path
