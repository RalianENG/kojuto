...
### Fixed
- **`/proc/self/maps` and `/proc/self/cgroup` re-enabled behind a dedup gate** — closes the libfaketime / LD_PRELOAD / container fingerprint blind spot that the previous exclusion opened. Both paths are back in `DefaultSensitivePaths` and re-classified by the analyzer's existing `isSandboxDetectionPath` as `CategoryEvasion`. The FP that motivated the original exclusion (glibc / V8 / Python runpy read these on every process launch, generating repeated MEDIUM events per scan) is now handled at the analyzer layer: `deduplicateEvasionByPath` collapses multiple CategoryEvasion events with the same FilePath into a single forensic breadcrumb, so the verdict rule (2+ MEDIUM to flip) measures DISTINCT sandbox-detection paths touched — not raw read repetitions. Path-only dedup (not (PID, FilePath)) is deliberate so cross-PID baselines (pip reading maps in one PID and node reading it in another) still collapse to one event; PID keying would treat every forked helper as fresh evidence and re-open the FP. Path-less evasion events like `ptrace(TRACEME)` are never deduped — they stack with sandbox probes as an independent modality. Effects: a package reading only `/proc/self/maps` stays clean (1 evasion event vs. 2+ MEDIUM threshold) but is now visible in the forensic report; a package reading maps + cgroup + status is flagged as an evasion cluster (3 distinct events).
- **Cosmetic path-form bypasses closed by capture-time normalization** — every captured filesystem path (openat, execve, execveat, unlink, unlinkat, rename, renameat, renameat2, dirfd-recorded base, tmp-file create tracker) now flows through `path.Clean` at the parser layer so `sensitivePathPatterns`, `isUserHomePath`, `isSystemBinaryWrite`, `isInstalledPackageWrite`, `suspiciousUnlinkDirs`, and the `createdTmpFiles` correlation cannot be dodged by cosmetic variants — `/etc/./shadow`, `/etc//shadow`, `/etc/foo/../shadow`, `/tmp/../etc/shadow`, `/home/dev/./.ssh/id_rsa` all collapse to their canonical form BEFORE the analyzer runs its substring check. Uses `path.Clean` (POSIX / forward-slash) rather than `filepath.Clean` (host-native separator) because kojuto's Windows and macOS builds parse strace lines captured from a Linux sandbox — the native filepath separator would corrupt them. Empty input is preserved (Clean("") returns "." which would fabricate a working directory). Symlink resolution is intentionally out of scope for the parser layer: the guest filesystem is attacker-controlled post-install, so any symlink-driven resolution would be attacker-authored; a separate analyzer-side policy (evasion event when a sensitive read is preceded by a suspicious symlink() call) is the correct locus and is tracked as follow-up. New tests: `TestPathNormalization_ClosesCosmeticBypasses` (six sub-cases covering `//`, `/./`, `/foo/../`, cross-root traversal, ssh-key case, idempotence), `TestPathNormalization_DirfdCompositionCollapses` (dirfd base + relative dot-segment), `TestPathNormalization_TrackTmpAndUnlink` (create/delete correlation across two different cosmetic forms).
- **openat dirfd bypass + strace filename escape unification** — the parser previously used a naive `"([^"]+)"` filename capture in every filename-carrying regex (openat/execve/execveat/unlink/unlinkat), which the earlier rename() fix already showed silently truncated on any path containing a literal `"` (rendered by strace as `\"`). That gap invisibly re-opened credential reads, home-dir writes, self-deletion, and payload exec against a payload named `/tmp/x"y`. Every regex now uses the alternation-escape form `(?:\\.|[^"\\])+` and every capture flows through the new `unescapeStracePath` helper (which delegates to the existing `unescapeStraceBuf`) so the analyzer sees the real filesystem path — one C-escape table, no divergence, no future \f-style silent bug. Separately, the openat parser now tracks fd → absolute path across strace lines via `ParseState.openFDs` and resolves `openat(<numeric-fd>, "<relative>", ...)` back to the absolute path. Without this, an attacker who opened `/etc/` once and then did `openat(fd, "shadow", O_RDONLY)` never tripped sensitive-path patterns (which match `/etc/shadow`, not the bare `shadow`) and the credential read was invisible. Per-PID scoping isolates one process's fd table from another; fd reuse after close() is handled by overwriting on the next successful open(); POSIX absolute-path-ignores-dirfd is honored. Boundary cases (unresolved dirfd from a pre-trace open, cross-PID leak attempt, failed open) are all pinned in `TestOpenatDirFDResolution*`.

### Added
- **Sandboxed + observed download phase** — `pip download` and `npm install --ignore-scripts` no longer run on the host. They now execute inside `sandbox.DownloadSandbox`, a hardened **network-enabled** container that mirrors every isolation control of the analysis sandbox (read-only rootfs, `cap-drop=ALL`, `no-new-privileges`, the restrictive seccomp profile, `pids-limit`, memory/cpu caps, the `kojuto.scan` label for orphan reaping) and differs only in keeping Docker's default network egress so pip/npm can reach the registry. A package-manager parser RCE or a path-traversal during tarball extraction is now contained to an ephemeral container instead of compromising the host before analysis even begins. The host staging directory is bind-mounted at `/out`; `writeSeccompFile` is shared with the analysis sandbox so both run the same seccomp profile.
  - The download runs under strace via `probe.NewContainerStraceForDownload`, and the captured events are stamped `types.PhaseDownload` and fed to the analyzer alongside the install/import events — the download phase is *observed*, not just contained. Events are drained concurrently with the traced command; if monitoring drops an event the download fails closed.
  - New `analyzer.classifyDownloadEvent` applies a download-specific profile. `--ignore-scripts` / `--only-binary` mean no package code should execute during download, so an `execve` out of the staging mount (`/out`) is an unpacking-time exploit → `code_execution` (HIGH). Writes that escape the staging dir into `$HOME` / system binaries / sensitive paths, `rename`-onto-system-binary, RWX memory, and `bind`/`listen` are judged by the same rules as install/import — those are the tarball-path-traversal escape signals and need no phase-specific handling.
  - New `CategoryDownloadEgress` (severity LOW): the download sandbox has real network egress, so a plain outbound connect is the phase doing its job (fetching from the registry and its CDN). It is recorded for forensic visibility but never flips the verdict alone — a bare-IP destination cannot be condemned without a trusted resolver, and a whitelist of registry domains was deliberately rejected. The whitelist-free heuristics still fire during download: a connect whose DNS query matches a known exfil service or shows tunneling entropy keeps its `data_exfiltration` / `dns_tunneling` category. Attributing a bare-IP C2 connect during download is left to follow-up synthetic-DNS-resolver work; until then containment bounds the blast radius.
  - `SyscallEvent.Phase` (previously declared but unused) is now populated for download-phase events; `types.PhaseDownload` joins `PhaseInstall` / `PhaseImport`.
...
### Changed
- **`downloader` public API returns download-phase events** — `Download`, `DownloadAll`, and `DownloadAllNpm` now also return the `[]types.SyscallEvent` captured while pip/npm ran inside the download sandbox; `Download` keeps returning the staging directory as its first value. `cmd/root.go` prepends these events to the scan timeline before `analyzer.Analyze`, so the report covers download → install → import. `downloadPackage`, `downloadBatchPackages`, and `downloadIndividually` thread the events through accordingly. The host `execCommand` seam in the `downloader` package is gone — every registry command now goes through the `runInSandbox` seam.
...
### Security
- **seccomp: block `io_uring_setup` / `io_uring_enter` / `io_uring_register`** — the pre-hardening `ALLOW`-by-default seccomp profile silently opened up every syscall Docker's stock profile blocks, most importantly the io_uring family. An attacker could submit `IORING_OP_OPENAT` / `OP_CONNECT` / `OP_SEND` / `OP_UNLINKAT` via a submission-queue entry, and the `sys_enter_*` tracepoints kojuto's strace and eBPF probes attach to would not fire because io-wq kernel workers dispatch the operations from an internal path. Blocking with `ERRNO` forces attacker code to fall back to conventional syscalls that kojuto's probes DO observe. Sandbox-aware payloads may pivot to declaring the environment a sandbox on `ENOSYS` — that is caught separately by the `CategoryEvasion` rules on `/proc` introspection.
- **seccomp: unblock `memfd_create`** — the previous profile blocked `memfd_create` with `ERRNO`, which made the analyzer's `parseExecveat` AT_EMPTY_PATH branch (added for fileless-exec detection) unreachable in the default configuration. Silently ENOSYS-blocking also gave sandbox-aware payloads a definitive fingerprint. Aligned with kojuto's "induce + observe" design principle: memfd_create is now allowed inside the sandbox and the subsequent `execveat(fd, "", ..., AT_EMPTY_PATH)` fires `code_execution` HIGH via the synthetic `/proc/self/fd/<n>` path.
- **evasion-test `d6_io_uring_setup_blocked`** — unconditional Group D probe that attempts `io_uring_setup(1, params)` via ctypes. Live-verifies the seccomp block; if the block regresses in a future edit, subsequent io_uring-based attacks re-open silently, so the probe serves as a canary the next scan will surface.
