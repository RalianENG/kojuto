...
func parseAuditHook(line string) (types.SyscallEvent, bool) {
	...
	// For import_attempt (emitted by the OS-simulation probe scripts, not by
	// the audit hook itself), payload is "<dist>:<module>:<result>"; the
	// event surfaces as an EventImportAttempt so analyzer.Analyze can decide
	// verdict = inconclusive when no import succeeded across the phase.
	if event == "import_attempt" {
		return parseImportAttempt(payload), true
	}
...
// parseImportAttempt decodes a `KOJUTO:import_attempt:<dist>:<module>:<result>`
// payload into an EventImportAttempt event. The result may be `ok` or
// `fail:<ErrorType>`; both are recorded so analyzer.Analyze can count
// attempts vs. successes across all import-phase runs and decide
// verdict = inconclusive when nothing succeeded.
//
// Payload shape (colon-separated):
//
//	<dist>:<module>:ok
//	<dist>:<module>:fail:<ErrorType>
//
// Dist and module are captured verbatim; the result is stored in
// FilePath ("ok" or "fail") and the failure ErrorType (if any) is
// stored in CodeSnippet so the report retains the diagnostic.
func parseImportAttempt(payload string) types.SyscallEvent {
	evt := types.SyscallEvent{
		Timestamp:  time.Now(),
		Syscall:    types.EventImportAttempt,
		AuditEvent: "import_attempt",
	}
	...
	if strings.HasPrefix(result, "ok") {
		...
		if strings.HasPrefix(result, "fail:") {
			evt.CodeSnippet = result[len("fail:"):]
		}
	}
	return evt
}
